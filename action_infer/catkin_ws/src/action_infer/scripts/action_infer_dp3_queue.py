...
dir = '/home/yxt/thesis/yirui/imitation/3D-Diffusion-Policy/3D-Diffusion-Policy/data/outputs/pick_place-0127-simple_dp3-8000-datav123-dot'
...
class ActionInfer:
    ...
    def get_next_file_index(self):
        pattern = re.compile(rf'recorded_traj(\d+)gripper{self.end_effector_length}dp3\.pkl')
    ...
    def save_graph(self):
        ...
        graph_output_path = os.path.join(self.traj_output_dir, f'height_time_graph{self.file_index}gripper{self.end_effector_length}dp3.png')
        print(f"Saving traj graph to {graph_output_path}")
        plt.savefig(graph_output_path)
        plt.close()
    
    def save_traj(self):
        traj_output_path = os.path.join(self.traj_output_dir, f'recorded_traj{self.file_index}gripper{self.end_effector_length}dp3.pkl')
        ...
        while not rospy.is_shutdown():
            if self.new_obs_received:
                with self.obs_lock:
                    self.new_obs_received = False
                with torch.no_grad():
                    action_dict, adopt_flag = self.model.predict_action(self.latest_obs)
                    
                adopt_flag = adopt_flag.detach().to('cpu').numpy()
                self.adopt_flag = adopt_flag[0]
...
@hydra.main(version_base=None, config_path="/home/yxt/thesis/yirui/imitation/3D-Diffusion-Policy/3D-Diffusion-Policy/diffusion_policy_3d/config", config_name="simple_dp3")
def main(cfg):
    try:
