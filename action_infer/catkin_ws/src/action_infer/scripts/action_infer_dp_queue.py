...
dir = '/home/yxt/thesis/yirui/imitation/3D-Diffusion-Policy/3D-Diffusion-Policy/data/outputs/pick_place-0127-simple_dp-8000-datav12'
# dir = '/home/yxt/thesis/yirui/imitation/3D-Diffusion-Policy/3D-Diffusion-Policy/data/outputs/pick-place-kinect-rs-img-dp-simple_dp-0209epoch8000_seed9'
...
class ActionInfer:
    ...
    def __init__(self, cfg: OmegaConf):
        ...
        self.queue_size = 2
        self.azure_img_queue = FixedSizeQueue(self.queue_size)
        self.rs_img_queue = FixedSizeQueue(self.queue_size)
        ...
        self.azure_rgb_sub = rospy.Subscriber(rgb_topic, Image, self.azure_rgb_callback, queue_size=1)
        
        rospy.init_node('action_infer', anonymous=True)

        self.model: DP3 = hydra.utils.instantiate(cfg.policy)
        self.end_effector_length = 0.08
        ...
        self.traj_output_dir = '/home/yxt/thesis/yirui/catkin_ws/experiment_imgs/0.08v2_banana'
    ...
    def get_next_file_index(self):
        pattern = re.compile(rf'recorded_traj(\d+)gripper{self.end_effector_length}dp\.pkl')
    ...
    def azure_rgb_callback(self, msg):
        rgb_image = self.br.imgmsg_to_cv2(msg)  
        output_shape = 300
        ...
        self.rgb_image = preproces_image(rgb_image)
        rgb_image = preproces_image(rgb_image)
        self.azure_img_queue.enqueue(rgb_image)
        if len(self.azure_img_queue.queue) == self.queue_size:
            self.new_azure_received = True
        else:
            self.new_azure_received = False
    ...
    def obs_callback_with_queue(self, msg): # manage the point cloud data via queue
        ...
        rs_img = preproces_image(rs_img)
        self.rs_img_queue.enqueue(rs_img)
        ...
        if len(self.rs_img_queue.queue) == self.queue_size:
            self.obs_full = True
    ...
    def save_graph(self):
        ...
        graph_output_path = os.path.join(self.traj_output_dir, f'height_time_graph{self.file_index}gripper{self.end_effector_length}dp.png')
        print(f"Saving traj graph to {graph_output_path}")
        plt.savefig(graph_output_path)
        plt.close()
    
    def save_traj(self):
        traj_output_path = os.path.join(self.traj_output_dir, f'recorded_traj{self.file_index}gripper{self.end_effector_length}dp.pkl')
    ...
    def lastest_obs_processing(self):
        ...
        while not rospy.is_shutdown():
            if self.obs_full and self.new_azure_received:
                azure_img1 = preproces_image(self.azure_img_queue.get_item(0)) # the oldest image
                azure_img2 = preproces_image(self.azure_img_queue.get_item(1)) # the newest image
                azure_img1 = torch.from_numpy(azure_img1).unsqueeze(0)
                azure_img2 = torch.from_numpy(azure_img2).unsqueeze(0)
                azure_img = torch.concat((azure_img1, azure_img2), axis=0).reshape(1, 2, 84, 84, 3).to(device)

                rs_img1 = preproces_image(self.rs_img_queue.get_item(0))
                rs_img2 = preproces_image(self.rs_img_queue.get_item(1))
                rs_img1 = torch.from_numpy(rs_img1).unsqueeze(0)
                rs_img2 = torch.from_numpy(rs_img2).unsqueeze(0)
                rs_img = torch.concat((rs_img1, rs_img2), axis=0).reshape(1, 2, 84, 84, 3).to(device)
                ...
                self.traj['agent_pos'].append(self.current_pose[0, 1, :])

                self.height_changes.append(self.current_pose[0, 1, 2])
                self.time_changes.append(elapsed_time)
...
@hydra.main(version_base=None, config_path="/home/yxt/thesis/yirui/imitation/3D-Diffusion-Policy/3D-Diffusion-Policy/diffusion_policy_3d/config", config_name="simple_dp")
def main(cfg):
    try:
